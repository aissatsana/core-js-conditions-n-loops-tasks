...
function convertToRomanNumerals(num) {
  const indexes = [10, 9, 5, 4, 1];
  const values = ['X', 'IX', 'V', 'IV', 'I'];
  let res = '';
  let number = num;
  let i = 0;
  while (number > 0 && i < indexes.length) {
...
function getSpiralMatrix(size) {
  ...
  while (value <= size ** 2) {
    ...
    if ((j !== maxSize && !reverse) || (j !== minSize && reverse)) {
      if (reverse) {
        j -= 1;
      } else {
        j += 1;
...
function sortByAsc(arr) {
  ...
  const offset = -minNumber;
  const count = new Array(maxNumber + offset + 1);
  ...
  for (let i = arr.length - 1; i >= 0; i -= 1) {
    const num = arr[i];
    sortedArr[count[num + offset] - 1] = num;
    count[num + offset] -= 1;
  }
...
function shuffleChar(str, iterations) {
  ...
  while (newIterations > 0) {
    ...
    if (newStr === str) {
      newIterations = iterations % count;
    }
  }
